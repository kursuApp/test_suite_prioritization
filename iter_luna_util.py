...
def get_edges(graph, current_node, test_case):
    ...
    while input_index < len(test_case):
        ...
        if not edge.empty:
            ...
            edges.append((edge.iloc[0]['source'], edge.iloc[0]['target'], edge.iloc[0]['key']))
        else:
            break
    return edges
...
def get_test_suite_by_edge(graph, start_node, test_suite):
    test_suite_by_edge = list()
    for index, test_case in enumerate(test_suite):
        test_suite_by_edge.append(get_edges(graph, start_node, test_case))
